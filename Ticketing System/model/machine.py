...
db = mysql.connect(host='localhost', port=3306, username='root', passw='root')
crsr = db.cursor()
crsr.execute("CREATE DATABASE IF NOT EXISTS tixsys")

def initiate_database():
    db = mysql.connect(host='localhost', port=3306, username='root', passw='root', database="tixsys")
    crsr = db.cursor()
...
def login_user():
    ...
    crsr.execute("""INSERT INTO tixsys_accounts (email, username, password, first_name,
